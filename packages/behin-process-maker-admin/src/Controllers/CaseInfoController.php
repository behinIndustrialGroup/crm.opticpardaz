...
class CaseInfoController extends Controller
{
    ...
    public static function get($caseId){
        ...
        return $data;
        return $data->sortBy('del_index')->toArray();
        $accessToken = AuthController::getAccessToken();
        $info = CurlRequestController::send(
            $accessToken, 
            "/api/1.0/workflow/light/participated/case/$caseId"
        ...
        $info = CaseController::getCaseInfo($caseId, $delIndex);
        $processId = $info->processId;
        return CaseTrackerController::get($processId);
        $taskId = $info?->currentUsers?->taskId;
        if($taskId){
            return GetTaskAsigneeController::listOfTaskAssignee($processId, $taskId);
        }
